...
1. Aegis-managed Dagu YAML 使用以下保留 labels：
   ...
   labels:
     ...
     aegis.folder.key: <sha256-hex-of-exact-grafana-folder-uid>
   ```

2. `aegis.folder.key` 是对原始 Grafana Folder UID UTF-8 字节计算 SHA-256 后的小写十六进制值，是 Folder
   ownership 的唯一生产事实。Dagu 2.13.0 会把用于查询和比较的 label key/value 归一化为小写，因此不能直接把
   可能大小写敏感的 Folder UID 作为 label value。公共 ID 中的 Tenant/Org 摘要继续作为兼容性和快速过滤，
   不再单独构成授权证明。
...
2. 读取或操作 Run、Step、Human Task、Approval、日志和 Artifact 时，Adapter 必须执行
   `Run -> Playbook ID -> Dagu YAML labels -> Folder key`，再与可信请求 Folder UID 的 SHA-256 key 比较。
...
1. 没有 `aegis.folder.key` 的现有 Org-scoped `pbk_` Playbook 不自动归给当前 Folder 或创建者。
...
1. `Playbook` 和 `PlaybookSummary` 增加 Provider-neutral `folder_uid`。Adapter 只有在 Dagu 原生 Folder key 与
   当前可信请求 Folder 匹配后才返回该请求 Folder UID；哈希值和 label key 不进入公共契约。
...
- Folder 删除后，reconciliation 通过枚举当前 Grafana Folder 并比较 key 识别仍有效的归属；没有任何匹配项的
  Playbook 作为 orphan fail-closed。哈希不可逆，因此 orphan 审计只记录 Folder key，不伪造原 UID。
