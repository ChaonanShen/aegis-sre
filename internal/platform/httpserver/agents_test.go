...
func TestCanvasTurnContextCarriesPublicSessionAndPublishRules(t *testing.T) {
	...
	if !strings.Contains(context, "session_id is ses_abcdefgh") || !strings.Contains(context, "node-exporter") || !strings.Contains(context, "query_prometheus range") || !strings.Contains(context, "canvas.publish_query_chart") || !strings.Contains(context, `"kind":"VizConfig"`) || !strings.Contains(context, `spec.options`) || !strings.Contains(context, `spec.fieldConfig`) {
