...
func NewHandler(service *canvasapp.Service, config Config) (http.Handler, error) {
	...
	mcp.AddTool(server, &mcp.Tool{Name: "canvas.publish_query_chart", Description: `Persist a bounded Prometheus range query and its Canvas chart definition for an Agent session. viz_config MUST be a VizConfig envelope: {"kind":"VizConfig","group":"timeseries","version":"v1","spec":{"options":{},"fieldConfig":{"defaults":{},"overrides":[]}}}. Put legend and tooltip in spec.options, field settings in spec.fieldConfig.defaults, and always keep spec.fieldConfig.overrides as an array; top-level Grafana panel options are invalid.`}, svc.publish)
